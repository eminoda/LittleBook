...
class BookController extends Controller {
	...
	async index() {
		...
		const field = ctx.query.sortField || '_id';
		const direction = ctx.query.sortDirection || 'asc';
		const list = await service.book.findList({}, { sort: { field, direction }, limit: pageSize, skip: (page - 1) * pageSize });
